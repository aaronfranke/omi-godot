...
func _import_node(state: GLTFState, gltf_node: GLTFNode, json: Dictionary, node: Node) -> Error:
	...
	if gltf_vehicle_body != null:
		gltf_vehicle_body.apply_to_nodes(state, node)
	return OK
...
func _get_node_extensions(node_json: Dictionary) -> Dictionary:
	var node_extensions = node_json["extensions"]
...
func _get_or_create_state_wheels_in_state(gltf_state: GLTFState) -> Array:
	...
	if state_extensions.has("OMI_vehicle_wheel"):
		omi_vehicle_wheel_ext = state_extensions["OMI_vehicle_wheel"]
	else:
		omi_vehicle_wheel_ext = {}
		state_extensions["OMI_vehicle_wheel"] = omi_vehicle_wheel_ext
		gltf_state.add_used_extension("OMI_vehicle_wheel", false)
	var state_wheels: Array
	if omi_vehicle_wheel_ext.has("wheels"):
		state_wheels = omi_vehicle_wheel_ext["wheels"]
	else:
		state_wheels = []
		omi_vehicle_wheel_ext["wheels"] = state_wheels
	return state_wheels
...
func _export_node(state: GLTFState, gltf_node: GLTFNode, json: Dictionary, _node: Node) -> Error:
	...
	if gltf_vehicle_body != null:
		...
		var node_extensions = _get_node_extensions(json)
